...
// Track cumulative kWh consumed in each of the 24 hours of the current day
let hourlyConsumption = Array(24).fill(0);

// Energy cost per kWh (in INR)
const energyCost = 5; // INR per kWh

// Alert thresholds
const HIGH_USAGE_THRESHOLD_KW = 5;      // kW
const HIGH_DAILY_THRESHOLD_KWH = 25;    // kWh
...
function handleAddDevice(e) {
    ...
    .then(newDevice => {
        // The server will assign the ID; avoid duplicates
...
function updateEnergyUsage() {
    ...
    let totalIncrementThisCycle = 0;
    devices.forEach(device => {
        if (device.isOn) {
            // Calculate usage based on actual time passed and device power (kWh)
            const usageIncrement = (device.power / 1000) * timeDiffHours;
            device.usage += usageIncrement;
            totalIncrementThisCycle += usageIncrement;
            usageUpdated = true;
    ...
    if (usageUpdated) {
        // Accumulate kWh into the current hour bucket
        hourlyConsumption[hour] += totalIncrementThisCycle;

        // Reset array at midnight (hour === 0 and we've crossed into a new day)
        if (hour === 0 && now.getMinutes() === 0 && now.getSeconds() < 5) {
            hourlyConsumption = Array(24).fill(0);
        }

        // Update chart dataset directly with kWh values
        if (energyChart) {
            energyChart.data.datasets[0].data[hour] = hourlyConsumption[hour];
            energyChart.update({ duration: 800, easing: 'easeInOutQuart' });
        }
...
function updateDashboard() {
    try {
        ...
        // High-usage alerts
        // Trigger alert only on rising edge
        if (currentUsage > HIGH_USAGE_THRESHOLD_KW && !highInstantAlertActive) {
            addAlert(`High instantaneous load: ${currentUsage.toFixed(2)} kW`, 'warning');
            highInstantAlertActive = true;
        }
        if (currentUsage <= HIGH_USAGE_THRESHOLD_KW * 0.9) {
            // Reset flag when usage drops sufficiently below threshold (with hysteresis)
            highInstantAlertActive = false;
        }
        if (dailyConsumption > HIGH_DAILY_THRESHOLD_KWH &&
            !alerts.some(a => !a.dismissed && a.message.startsWith('High daily'))) {
            addAlert(`High daily consumption: ${dailyConsumption.toFixed(2)} kWh`, 'warning');
        }
